...
adminRoute.put("/edit-property", async (req, res) => {
  try {
    const { propertyId, title, price, area, description, location } = req.body;
    const { pic } = req.files;
    pic.mv("uploads/" + pic?.name, (err) => {
      if (err) {
        res.json({
    ...
    const result = await propertyModel.findByIdAndUpdate(
      propertyId,
      { title, price, area, description, location, pic: pic?.name,},
      { new: true }
    );
    console.log(result);
    if (result) {
...
adminRoute.post("/contact-us-list", async (req, res) => {
  try {
    ...
    const finalData = await Promise.all(
      data.map(async (item) => {
        const contactData = await contactModel.findOne({ _id: item?._id });
        return contactData;
      })
    );
    res.json({
